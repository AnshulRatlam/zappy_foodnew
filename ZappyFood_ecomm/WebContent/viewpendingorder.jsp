 ...
 <%
   for(Order_Bean ee:list)
   {
	...
	<td scope="row"> <%=ee.getQuantity() %>  </td>
	 ...
	 <td><a href="orderDispatch?oid=<%=ee.getOid()%>&action=1&uid=<%=ee.getUser()%>" class="glyphicon glyphicon-edit"></a></td>
	 
	 <td><a href="orderDispatch?oid=<%=ee.getOid()%>&action=2&uid=<%=ee.getUser()%>" class="glyphicon glyphicon-remove-sign" onClick="return confirm('Do you really want to delete this record?')"></a></td>
	 <tr/>
	 <%
	 }
  %>
